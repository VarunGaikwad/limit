...
import { Plus, Coffee, ShoppingCart, Activity, ShoppingBag, Car, Home as HomeIcon, Smartphone, Zap, Briefcase, PlusCircle, X, Edit2, Trash2 } from "lucide-react";

const INITIAL_CATEGORIES = [
  { id: 1, name: "Food & Drink", type: "expense", icon: Coffee, color: "text-orange-500", bg: "bg-orange-500/10", budget: 400 },
  { id: 2, name: "Groceries", type: "expense", icon: ShoppingCart, color: "text-emerald-500", bg: "bg-emerald-500/10", budget: 600 },
  { id: 3, name: "Entertainment", type: "expense", icon: Activity, color: "text-purple-500", bg: "bg-purple-500/10", budget: 150 },
  { id: 4, name: "Shopping", type: "expense", icon: ShoppingBag, color: "text-pink-500", bg: "bg-pink-500/10", budget: 300 },
  { id: 5, name: "Transport", type: "expense", icon: Car, color: "text-blue-500", bg: "bg-blue-500/10", budget: 200 },
  { id: 6, name: "Housing", type: "expense", icon: HomeIcon, color: "text-indigo-500", bg: "bg-indigo-500/10", budget: 1500 },
  { id: 7, name: "Utilities", type: "expense", icon: Zap, color: "text-yellow-500", bg: "bg-yellow-500/10", budget: 250 },
  { id: 8, name: "Subscriptions", type: "expense", icon: Smartphone, color: "text-rose-500", bg: "bg-rose-500/10", budget: 80 },
  { id: 9, name: "Salary", type: "income", icon: Briefcase, color: "text-[#00b88e]", bg: "bg-[#00b88e]/10", budget: 0 },
];

export default function Category() {
  const { setTitle, setSubtitle, setTopContent } = useDashboard();
  const [isModalOpen, setIsModalOpen] = useState(false);
  const [editingCategory, setEditingCategory] = useState<any>(null); // null means adding a new category
    ...
    setTopContent(
      <div className="w-full mt-4 md:mt-2">
        <div className="flex flex-col sm:flex-row gap-4 justify-between items-start sm:items-center bg-white p-6 rounded-3xl shadow-[0_8px_30px_rgb(0,0,0,0.04)] border border-slate-100">
          <div>
            <h3 className="text-2xl font-bold text-slate-800 tracking-tight">Organize Finances</h3>
            <p className="text-sm text-slate-500 font-medium mt-1">Add or edit tags used for mapping your transactions.</p>
          </div>
          <button 
            onClick={() => {
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-3 sm:gap-4">
          {INITIAL_CATEGORIES.map((cat) => (
            ...
            >
              <div className="flex items-center gap-4 position-relative z-10">
                <div className={`p-3.5 rounded-[1.25rem] flex items-center justify-center transition-transform group-hover:scale-110 ${cat.bg} ${cat.color}`}>
                  <cat.icon size={22} strokeWidth={2.5} />
                </div>
                <div>
                  <h4 className="font-bold text-slate-800 text-base sm:text-lg leading-tight transition-colors">{cat.name}</h4>
                  <div className="flex items-center gap-2 mt-0.5">
                    <span className="text-[11px] sm:text-xs font-bold uppercase tracking-wider text-slate-400">
                      {cat.type} {cat.type === 'expense' && cat.budget > 0 && `• $${cat.budget}/mo`}
                    </span>
                  </div>
                </div>
              </div>
          ...
          >
            <div className="p-3.5 rounded-[1.25rem] flex items-center justify-center bg-transparent group-hover:bg-primary/10 transition-colors">
              <PlusCircle size={22} className="group-hover:scale-110 transition-transform" strokeWidth={2.5} />
            </div>
            ...
            <div className="space-y-6">
              <div className="space-y-2">
                ...
                <p className="text-slate-500 text-sm">
                  {editingCategory ? "Update the details for this tag." : "Create a custom tag for your transactions."}
                </p>
              </div>

              <div className="space-y-4">
                <div className="space-y-1.5">
                  <label className="text-sm font-semibold text-slate-600 pl-1">Category Name</label>
                  <input 
                    type="text" 
                ...
                <div className="flex gap-4 w-full">
                  <div className="space-y-1.5 flex-1">
                    <label className="text-sm font-semibold text-slate-600 pl-1">Type</label>
                    <select 
                      defaultValue={editingCategory?.type || "expense"}
                      className="w-full bg-slate-50 border border-slate-200 rounded-2xl px-5 py-3.5 text-slate-800 focus:outline-none focus:ring-2 focus:ring-primary/50 focus:border-primary transition-all appearance-none cursor-pointer"
                    >
                  ...
                  <div className="space-y-1.5 flex-1">
                    <label className="text-sm font-semibold text-slate-600 pl-1">Monthly Budget ($)</label>
                    <input 
                      type="number" 
